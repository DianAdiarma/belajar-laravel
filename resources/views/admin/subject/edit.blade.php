            ...
            <div class="form-group">
              <select class="custom-select" id="inputGroupSelect01" name="lecturer_id">
                    ...
                    @foreach ($lecturers as $lecturer)
                        <option @if ($subjects->lecturer_id == $lecturer->id) selected @endif value="{{$lecturer->id}}">{{$lecturer->name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
              <label for="exampleInputcode">Kode Mata Kuliah</label>
              <input value="{{$subjects->code}}" type="text" name="code"class="form-control" id="exampleInputcode" placeholder="enter your Kode Mata Kuliah">
              
            </div>
            <div class="form-group">
              <label for="exampleInputname">Name Mata Kuliah</label>
              <input value="{{$subjects->name}}" type="text" name="name" class="form-control" id="exampleInputname" placeholder="enter your Nama Mata Kuliah">
            </div>
